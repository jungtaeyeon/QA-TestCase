...
- **[대분류] 번호 조합 구성 요소 설정**
  ...
  - [중분류] 자유 텍스트 설정
    ...
    - 최솟값(0글자 허용 여부) 및 빈값 저장 처리 [SC-CASEID-021] ✅
  - [중분류] 일련번호 자릿수 설정
...
| 번호 조합 구성 요소 설정 | 21 | 21 | 0 | 0 | 0 | 100% | 100% |
...
| SC-CASEID-021 | 자유 텍스트 최솟값·빈값 저장 처리 | TC-CASEID-021-001 | ✅ 완료 |
...
**TC 게이트 판정: ✅ PASS** — ❌ 0건, ⏸️ 0건

> TC 통계: 총 98건 | Smoke 12건(12.2%) | E2E 13건(13.3%) | Functional 50건 | Negative 22건 | Boundary 16건
...
|---|------|----------------|--------|------|
| 1 | 자유 텍스트 최솟값: 0글자 허용 여부 및 빈값(공백·미입력) 저장 시 처리 방식 | SC-CASEID-021 / REQ-025 | TC 작성 완료 | ✅ 해결: 공란 저장 시 칩 제거된 상태와 동일 (칩 미등록) → TC-CASEID-021-001 |
...
| 3 | Destination 미확정 상태 C.1.1 표시 방식 | SC-CASEID-046 / REQ-053 | TC 업데이트 완료 | ✅ 해결: 확정 전 미표시 → TC-CASEID-046-001 title·Expected Results 갱신 |
...
| 2026-05-08 | tc-reviewer (자동) | MISSING-010/CONFIRM-003/MISSING-006 해결 반영 — SC-CASEID-021 TC 추가, TC-CASEID-027/046 갱신, 총 98건 |
...
| 자유 텍스트 | 최대 6글자, 영문/숫자, 최솟값: 0글자 허용 (빈값 저장 시 칩 미등록 상태와 동일) |
